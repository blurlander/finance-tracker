...
const ExpenseItem = ({ color, amount, category, text, index, userId, categoryList }) => {
  ...
  const handleDeleteExpense = async (e) => {
    ...
    if (confirm(text) == true) {
      ...
      const response = await fetch('https://finance-tracker-api-c4uj.onrender.com/api/deleteExpense', {
        method: 'POST',
  ...
  const handleEdit = async (e) => {
    ...
    const response = await fetch('https://finance-tracker-api-c4uj.onrender.com/api/editExpense', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
